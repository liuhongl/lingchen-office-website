      ...
      <main>
        ...
        <section className="deepdoc-challenges">
          <div className="deepdoc-shell">
            ...
            <div className="deepdoc-challenge-grid">
              <article><div className="deepdoc-challenge-index"><b>1</b><i /></div><h3>挑战一：单证类型极其多样</h3><p>涵盖基础商业单证（合同、PO单、商业发票、装箱单）、货运物流单证（海运/空运提单、船证明、仓单）、官方监管单证（报关单、原产地证、检验证书）、金融信用单证（信用证 MT700/710、汇票、保单等10+类型）。每种单证都有独特结构和关键字段，需系统具备广谱识别能力。</p></article>
              <article><div className="deepdoc-challenge-index"><b>2</b><i /></div><h3>挑战二：版式无标准，千差万别</h3><p>不同国家、不同银行、不同公司都有自己的单证模板，同一类型单证存在数十种版式变体。传统 OCR 模式极度依赖固定模板标注，泛化能力差，遇到新版式即失效。传统标注训练方式成本高、效果差，无法适应真实业务场景。</p></article>
              <article><div className="deepdoc-challenge-index"><b>3</b><i /></div><h3>挑战三：抽取需求高度复杂</h3><p>涉及表格信息抽取（货物描述、明细、单价、总价）、条款内容理解（信用证特殊条款、合同权利义务）、逻辑关系判断（单证间关联性、数据一致性）、专业术语解析（贸易术语、行业特定表达）。需要系统具备语义理解和逻辑推理能力，而不仅是文字识别。</p></article>
            </div>
          </div>
        </section>
        ...
        <section className="deepdoc-cooperation">
          <div className="deepdoc-shell"><p className="deepdoc-pill"><Image src="/images/deepdoc/cooperation-pill-icon.png" alt="" width={16} height={16} />合作模式</p><h2>懂业务 · 经验丰富 · 服务健全的全栈 AI 升级赋能</h2><p className="deepdoc-cooperation-lead">提供标准产品开箱即用、本地化部署安全可控、轻量级定制快速适配以及综合场景定制深度专属服务。</p>
            <div className="deepdoc-cooperation-grid"><article><h3><Image src="/images/deepdoc/service-icon.png" alt="" width={20} height={20} />合作服务模式</h3><div><b>标准产品开箱即用 / 订阅服务</b></div><div><b>本地私有化部署 / 深度专属定制</b></div></article><article className="active"><h3><Image src="/images/deepdoc/payment-icon.png" alt="" width={20} height={20} />付费结算方式</h3><div><b>产品订阅年费 + 弹性调用流量套餐</b></div><div><b>产品部署与定制开发费 + 年度维护与规则微调/升级服务费</b></div></article></div>
