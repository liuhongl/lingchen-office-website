...
export default function DeepLawPage() {
  return <div className="deeplaw-page">
    ...
    <main>
      ...
      <section className="deeplaw-stages deeplaw-section"><div className="deeplaw-shell"><SectionHeading title="法律业务六大核心环节"/><div className="deeplaw-stage-grid">{legalStages.map((x,index)=><div className="deeplaw-stage-item" key={x[0]}><article><b>{x[0]}</b><h3>{x[1]}</h3><p>{x[2]}</p></article>{index % 3 !== 2 && <StageArrow/>}</div>)}</div></div></section>
