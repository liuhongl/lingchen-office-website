...
const details = [
  ...
  ["3.5", "跨境单证智能审核 · 合规履约与风控", "新一代跨境单证智能审核——构建会思考、会进化的国际单证智能审核大脑", "跨境成交涉及复杂单证，人工审单慢、培养难，不符点拒付风险高，声誉承压。", "大模型 + OCR 双引擎、知识工程、四维审核体系、零样本/少样本抽取、自然语言配置规则。", "从「执行规则」到「具备认知」的范式革命，大幅降低贸易拒付风险，保障安全。", ["处理效率提升 5~6 倍", "关键信息审核准确率 90%+", "审单员培养 36 月 缩短至 1 月"]],
];

const metrics = [["500%+", "触达 / 作业效率提升"], ["40%~60%", "综合通联/作业成本降至"], ["90%+", "单证关键信息审核准确率"], ["36个月→1个月", "审单人才培养周期缩短"], ["7×24小时", "跨时区获客与履约不漏接"]];
...
export default function AiOverseasSolutionPage() {
  return <>
    ...
    <main className="overseas-page">
      <section className="overseas-hero"><div className="shell"><small><i />出海获客与合规 · 行业解决方案</small><h1>AI 出海获客 <span>全链路解决方案</span></h1><h3>五大 AI 智能体 · 一条增长闭环</h3><p>为本方案以「出海企业如何在陌生市场高效率、持续、低成本地获取并赢得客户」为主线，从「数据挖掘与筛选」到「被 AI 看见」再到「成交履约」，让出海获客成为可经营、可衡量、可进化的能力体系。</p><BookingTrigger className="button">预约产品演示 <ArrowRight size={16} /></BookingTrigger></div></section>

      <section className="overseas-section"><div className="shell"><SectionTitle no="一">出海获客之困：行业与企业面临的核心问题</SectionTitle><p className="overseas-lead">出海企业在陌生市场、跨时区运营、合规高压与本地化门槛之下，面临的是一个「入口 — 线索 — 转化 — 履约」环环相扣的系统工程：</p><div className="pain-grid">{pains.map((x,i)=><article key={x[0]}><div className="pain-card-heading"><b>0{i+1}</b><h3>{x[0]}</h3></div><strong>{x[1]}</strong><p>{x[2]}</p></article>)}</div><div className="overseas-ribbon">问题的本质：出海获客的本质，是把「入口被看见 → 线索被发现 → 客户被转化 → 成交被履约」这条断点重重的链路，重建为一条可经营、可衡量、可进化的增长闭环。</div></div></section>

      <section className="overseas-section overseas-soft"><div className="shell"><SectionTitle no="二">解决方案全景：五大智能体，一条增长闭环</SectionTitle><p className="overseas-lead">灵宸智能以「统一知识中枢 + 数据飞轮」为底座，将五个 AI 智能体按获客链路首尾相连、各司其职，覆盖从品牌入口到成交履约的全过程。</p><div className="agent-overview">{agents.map(([Icon,tag,title,text])=><article key={String(title)}><div className="agent-overview-heading"><Icon size={28}/><small>{String(tag)}</small></div><h3>{String(title)}</h3><p>{String(text)}</p></article>)}</div></div></section>
