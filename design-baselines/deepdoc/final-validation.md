...
- 图标：`evidence-pill-icon.png` 已替换为 MasterGo 当前图层 `33:060871` 资源面板生成的 50×50 PNG，CSS 16×16、有效 DPR 3.125，清晰度问题已修正；但该文件不是官方 SVG 下载结果，来源门禁仍为 `BLOCKED`。其余页面专属 PNG 仍来自 100% MasterGo 截图单倍裁切，完成逐图层原始 SVG 或至少 2× 位图替换前为 `BLOCKED`。
...
- 仍存素材门禁：历史页面专属位图尚未全部替换为 MasterGo 原始 SVG 或至少 2× 正式位图，因此不勾选“已视觉校准”或“1:1 验证通过”。
...
- [ ] `pnpm mastergo:check deepdoc` 未通过：历史页面专属位图仍缺少 MasterGo 原始 SVG 或至少 2× 正式位图，视觉台账保持 `BLOCKED`；本轮不宣称严格 1:1。
...
- [ ] 历史素材 `BLOCKED` 状态不变，因此本轮不宣称 1:1 验证通过。
