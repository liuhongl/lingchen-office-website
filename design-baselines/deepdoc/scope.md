...
- 用户明确取消 Footer 后用于补齐旧 MasterGo 根画板总高的 `520px` 浅灰尾部留白；该要求覆盖本文件早期“保留设计留白”的历史例外。
- 本轮只删除 DeepDoc 页面专属 `.deepdoc-tail` 结构与样式，不修改冻结公共 `SiteFooter`。
- 目标视口为桌面端 `1576×1258`；移动端继续不在当前官网范围。
