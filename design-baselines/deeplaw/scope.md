...
- Hero 主标题去掉独立宋体字体效果，改为继承 DeepLaw 页面无衬线字体栈；字号、字重、行高、渐变和布局不变。
- 删除 Hero 顶部“产品定位”标签。
...
- 业务流、六大环节与核心能力卡进一步取消固定最小高度，由内容与等量上下 padding 撑高。
- 六大环节自迭代引擎由深色改为浅色提示框。
- 用户再次确认：Hero 以外全部 `.deeplaw-section` 统一为 `60px` 上下内边距；原约 `92px` 的共享规则不再适用。

## 2026-07-25 `/solutions/products/deeplaw/` 浏览器批注范围

- 内容真源：用户本轮 15 条浏览器批注；仅替换“分钟级合同审查”并控制明确换行，其余正文不改写。
- 视觉真源：用户提供的 1576×1258 浏览器选区截图与当前 DeepLaw 官网实现。
- 允许修改：DeepLaw 页面 JSX、页面级 `prototype-sync.css` 与本页台账。
- 冻结资产：公共 Header、Footer、Logo、BookingModal、BookingTrigger 不修改。
- 桌面端范围：Hero 标题两行且不侵入右侧轨道；同类卡片由内容和 20px 上下内边距撑高；CTA 为 60px 上下内边距。
- 移动端不在当前官网验收范围。

## 2026-07-27 浏览器批注第三批

- 用户覆盖：主 Agent 的五张模块卡等高；Agents 区导语的 `800px` 文本容器整体水平居中。
- 目标视口 `1576×1258`；只修改本页 `prototype-sync.css` 与证据台账，可见文案、图标和公共资产不变。

## 2026-07-27 浏览器批注第四批

- 覆盖 `/products/deeplaw/` 的对比卡、五张核心能力卡、工作流优势面板、增长闭环正文及同类价值卡 `20px` 内边距；目标视口 `1576×1258`。
- 可见文案与正式图标不变；公共资产冻结，移动端不在本批范围。

## 2026-07-27 浏览器批注第六批

- Hero 仅收紧纵向间距，不删除、改写或隐藏内容。
- 五张 `.deeplaw-capability-grid` 卡的“核心能力 N”与标题统一同排。
- 两张 `.deeplaw-cases` 场景卡四边内边距 `20px`、内容驱动高度、同一 Grid 行等高。
- 目标视口 `1576×1258`；公共资产冻结，移动端不在范围。
