...
|---|---|---|---|---|---|
| HOME-V-20260727-01 | “为您的商业结果服务” | `16px` | 现有的 120% | `19.2px` | READY |
| HOME-V-20260727-02 | 双使命文字 | `15px`；文字外边界 `594.5–876.5` | 放大并与按钮外边界 `551–920` 对齐 | `18px`；容器宽 `369px`；两端分布 | READY |

## 2026-07-27 Hero 间距复核覆盖

| 视觉 ID | 元素 | 修改前实测（1486×1258） | 用户覆盖目标 | 实现要求 | 状态 |
|---|---|---|---|---|---|
| HOME-V-20260727-03 | Agent 徽章横向留白 | 第二行宽 `172.8px`；徽章宽 `190px`；两侧约 `8.6px` | 字体放大后保留合适左右间距 | 徽章宽 `222px`，两侧约 `24px`；说明列宽保持约 `445px` | PASS；实测两侧均 `24.59px` |
| HOME-V-20260727-04 | 右侧“灵宸 AI Agent” | `14px / 700` | 字体调大 | `16px / 700`；间距与颜色不变 | PASS |
| HOME-V-20260727-05 | 第一条使命圆点 | 圆点左边界约 `535px`，按钮左边界 `551px` | 圆点与按钮左边界对齐 | 第一项增加 `16px` 左内距，圆点 `left:0` | PASS；圆点与按钮左边界均为 `551px` |
