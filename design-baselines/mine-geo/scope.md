# Mine GEO MasterGo 范围台账

## 2026-07-22 原型内容同步批次

- 内容真源：Google AI Studio 当前交互原型 `https://ai.studio/apps/31940409-b4c4-420a-8018-a504586355a9`，以页面主体 `<main>` 的实际渲染文本为准。
- 视觉真源：本目录既有 MasterGo 基线、当前 `/products/mine-geo/` 已实现页面与冻结公共设计系统。
- 用户覆盖：执行 `1A / 2A / 3A`；有对应关系的大模块保留现有视觉，只同步模块内容、顺序、文案、指标和主体 CTA；明显错字修正，歧义保留原型。
- 当前范围：仅桌面端；移动端未纳入本轮官网实现和验收范围。
- 冻结资产：`SiteHeader`、`SiteFooter`、Logo、`ProductBreadcrumb`、`BookingModal`、`BookingTrigger`。
- 本节覆盖下方历史 MasterGo 内容真源口径；历史视觉证据继续有效。

- 页面名称：品牌生成式引擎优化与获客 AI Agent
- MasterGo：`https://mastergo.com/file/198643424708731?fileOpenFrom=team&page_id=33%3A088500&devMode=true`
- `page_id`：`33:088500`
- 原始画板：1440×6959，SHA-256 `f789ec245e236107b20f03c2818b4e591f7a9f8a3d572a7f0896554ae50ac346`
- 实现路由：`/products/mine-geo/`
- 范围：整页；目标桌面视口 1440×6959。
- 公共 `SiteHeader`、`SiteFooter`、Logo、`BookingTrigger`、`BookingModal` 冻结复用。

## 公共资产基线

| 资产 | 开工 SHA-256 | 允许变化 |
|---|---|---|
| Header | `a5edbe507ea4338ccf61f7c50018a2c6863e3fdfebf7b47ac9603c34cfd033ec` | 既有路由高亮 |
| Footer | `5816ee5da601620216059640fcaace7f3f25c030d8280423f42e08759ddec20b` | 无 |
| Logo | `badc568e402a6faa849022806329283acd678e333cc0ce892296220abf8fdf11` | 无 |
| BookingModal | `1ef9eb10296ff62a52157d0afc0f92d63c321655a30f746d537a08abafa26842` | 无 |
| BookingTrigger | `cf6bfae2e7035ac9fe65819ab15dd5ab0eed4e2cd18aee2bd3d652d28bc4fad7` | 无 |

## 用户确认

- 整页还原；禁止补写文案、禁止近似图标。
- 公共头尾冻结。
- 本页在 Social Grow 门禁通过后开始。
- 2026-07-20 用户确认保留当前 Header；交付冻结基线更新为 `ea5b1626042d5b6e8d26e7b4c878d27f7301d51e41cd3f82058c06196b065227`，原始开工哈希继续保留用于追溯。
- 2026-07-21 用户确认优化四张指标卡的重复箭头：保留 MasterGo 正式方向图标，移除标题文字中的 `↑/↓`，其余指标文案不变。
- 2026-07-21 并发接入的共享 `ProductBreadcrumb` 默认高度为 74px，和本页 46px 设计基线冲突；本页使用不影响其他页面的 `compact` 变体恢复原高度、分隔符和无额外 CTA 的设计状态。
- 2026-07-23 用户确认删除 Hero 顶部“产品定位”标签；本页继续使用 compact 面包屑，不新增预约按钮。
- 2026-07-23 用户确认将“五大能力 · 一个闭环”（含横向箭头）、“重构关键业务流”、“核心产品能力”、“产品价值与实效指标归因”和“品牌生成式引擎优化与获客 AI Agent”按独立大模块展示，并在各模块底部保留明确间距；本次只调整 Mine GEO 主体分区，不改文案、卡片内容和公共组件。
- 当前官网仅验收桌面端，移动端未纳入本轮范围。

## 2026-07-24 场景表用户覆盖

- “典型应用场景与成功案例”表格在“适用角色”后新增“痛点”列。
- 原末列中的“【痛点】…【收益】”拆为痛点和收益两列；末列删除“【收益】”标签，正文保持逐字不变。
- 其余 Mine GEO 模块、公共 Header/Footer 与正式图标不在本轮修改范围。

## 2026-07-24 Hero 与闭环间距覆盖

- 目标视口：`1616×1258`；只修改 Mine GEO Hero 与“五大能力 · 一个闭环”两个主体区块。
- Hero 删除固定高度，以内容高度和上下 `60px` 内边距撑开；不修改标题、正文、对比卡或正式图标。
- “五大能力 · 一个闭环”删除固定高度并将上下内边距统一为 `60px`，横向闭环线仍归属该 section。
- 公共 Header/Footer、Logo、ProductBreadcrumb、BookingModal、BookingTrigger 保持冻结。

## 2026-07-24 传统方案表格拆列

- 只修改“相比传统方案”表格：由两列改为「验证维度 / 传统做法 / Mine GEO 闭环」三列。
- 三行内容仅按原有 `【传统做法】` 与 `【Mine GEO 闭环】` 标签拆分，不改写原文。
- 用户再次确认：Hero 以外全部顶层正文 section 统一为 `60px` 上下内边距。

## 2026-07-25 浏览器批注范围

- Hero 标题同排、两段正文扩宽；能力、优势、结果卡统一图标标题行。
- 痛点卡、场景行、路径卡、价值卡、合作卡删除固定/最小高度。
- Closing 删除固定高度、上下 `60px`，标题按用户指定换行；目标视口 `1576×1258`。

## 2026-07-26 20:27 浏览器纯文字批注范围

- 证据仅来自用户提供的纯文字要求、当前源码、当前 DOM 与计算样式；不读取、不复用旧任务图片。
- BC-07：`#mgs-capabilities-title` 逐字改为 `一、 五大能力 · 一个闭环`。
- BC-08：桌面端主体 section 的左右内容边界统一对齐当前公共 Header shell（实测宽 `1200px`）。
- BC-09 / BC-10：核心功能三、四当前均为 `height:auto`，`scrollHeight === clientHeight`，只做回归，不重复修改。
- BC-11：仅模式 1 RaaS 卡取消网格拉伸并最小收紧上下内边距；其余合作卡不变。
- BC-12：仅模式 2 将“托管运营服务”标题与“模式 2”徽标放在同一标题行。
- BC-05 同页影响：已盘点的区块级浅色 lead / subtitle / intro 改为 `#111827`，范围见 `design-baselines/solution-batch/light-text-inventory-2026-07-26.md`。
- 目标桌面视口 `1576×1258`；移动端未纳入范围；公共资产继续冻结。

## 2026-07-26 浏览器批注第二批

- 用户覆盖 ID 17–19、26–28：成果标题与图标缩小且标题不换行；场景表上下居中；三张合作卡标题均跟在模式标签后；三卡内容驱动、上下内边距一致。
- 新覆盖取代“仅模式 2 同排”和“模式 1 单独 16px padding”的旧局部范围；文案与公共资产不变。

## 2026-07-27 浏览器批注第三批

- 用户覆盖：优势卡标题缩小并保持单行、同组图标同步缩小；场景表减少纵向留白；三张合作卡等高。
- 目标视口 `1576×1258`；可见文案、图标文件和公共资产不变。

## 2026-07-27 浏览器批注第四批

- 覆盖 `/products/mine-geo/` Hero 双色对比大模块、工作流标题上距、合作卡付费行对齐与同类价值卡 `20px` 内边距。
- 目标视口 `1576×1258`；文案、图标和公共资产不变，移动端不在范围。

## 2026-07-27 浏览器批注第五批

- 仅覆盖 `/products/mine-geo/`：Hero 背景衔接、两张对比卡间距及图标白底、五张能力卡编号与正文上距、四张工作流卡正文上距。
- 目标视口 `1576×1258`；不修改可见文案、不替换现有正式图标文件，不改公共 Header/Footer/Logo/BookingModal/BookingTrigger。
- 移动端未纳入本轮官网范围。
