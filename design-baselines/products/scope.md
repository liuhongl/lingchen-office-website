...
## 2026-07-24 卡片色彩与 Hover 最新覆盖

- 内容真源：现有 `content-ledger.md`，本轮不修改卡片文案、顺序、链接或指标。
- 视觉真源：用户指定 Google AI Studio 原型，2026-07-24 从预览 iframe 的实际样式表与交互态取证。
- 最新用户覆盖优先于 2026-07-22 的“统一白底”历史覆盖：前五张卡片使用出海类蓝色体系；Recov AI 使用黄色体系；DeepLaw 使用绿色体系；各自 Hover 使用原型对应深色。
- 允许修改：`app/products/page.tsx` 的卡片视觉分类 class；`app/globals.css` 的产品卡片默认态、Hover / focus-within 色彩与文字适配。
- 禁止修改：卡片内容、顺序、链接、网格结构、公共 Footer、Logo、BookingModal；公共 Header 只应用用户同轮明确授权的全站品牌名称与导航字号调整。

## 2026-07-26 卡片密度覆盖

- 内容真源不变；不修改七张卡片文案、顺序、链接、颜色分类和交互。
- 用户明确要求全部产品卡片圆角改为 `5px`，上下内边距改为 `20px`。
- 用户要求正文和底部信息区上方留白进一步收紧；本轮统一由 `24px` 调整为 `16px`，保持七卡一致。
- 允许修改：`.product-card`、`.product-card__description`、`.product-card__footer` 的共享桌面视觉样式。
- 禁止修改：网格宽度、列数、色彩体系、字体、公共 Header/Footer/Logo/BookingModal。
- 目标桌面视口：`1576 × 1258` CSS px；移动端不在本轮范围。
- 开工 SHA-256：产品页 `eefc0245dfe8104639105ddeb5ad73778232b810548dfbd8f2691367554e80ea`；全局样式 `f2a4a1a9a81bd5729aab4087eee3735838e20824851b80ccb9f6adce14fbb6f2`。

## 2026-07-24 宽版矩阵覆盖

- 用户确认产品矩阵内容区不再受 1200px 上限约束，桌面端目标宽度放宽至 1440px，使三列卡片呈横向长方形。
- 只调整矩阵容器宽度，不修改卡片文案、顺序、链接、颜色分类和交互。
