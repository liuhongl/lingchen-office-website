...
- 合作模式说明保持单行。
- 用户再次确认：全部 `.recov-section` 统一为 `60px` 上下内边距，Hero 除外。
- “商业实效”模块使用独立浅蓝背景，与前后模块形成明确分区。

## 2026-07-27 浏览器批注第四批

- 覆盖 `/products/recov-ai/` 压力卡序号色、四张业务流正式图标显示与同类价值卡 `20px` 内边距。
- 继续使用已登记 MasterGo 正式 SVG；只修复裁切与显示尺寸，不替换素材。目标视口 `1576×1258`，公共资产冻结。

## 2026-07-27 浏览器批注第六批

- 仅覆盖 `.recov-case-grid` 两张典型场景卡：四边内边距 `20px`、内容驱动高度、同一 Grid 行等高。
