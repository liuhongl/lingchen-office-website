...
- 内容真源：用户提供的 28 篇 DOCX、已审计结构化文章数据及本目录内容台账。
- 视觉真源：MasterGo 研究院列表画板、前 6 张正式卡片视觉和用户确认的列表扩展覆盖规则。
...
- 2026-07-25 用户确认分类必须以两个正式素材文件夹为准；第 4 篇 RaaS 的源目录属于“行业洞察”，其卡片标签与详情分类同步纠偏。
- 当前筛选统计为：行业洞察 10、解决方案&白皮书 10、产品动态 8。
